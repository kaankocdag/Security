...
RUN dotnet restore src/Kaan.SecurityPlatform.ScannerWorker/Kaan.SecurityPlatform.ScannerWorker.csproj

COPY . .
RUN dotnet publish src/Kaan.SecurityPlatform.ScannerWorker/Kaan.SecurityPlatform.ScannerWorker.csproj -c Release -o /app --no-restore
...
ENTRYPOINT ["dotnet", "Kaan.SecurityPlatform.ScannerWorker.dll"]
